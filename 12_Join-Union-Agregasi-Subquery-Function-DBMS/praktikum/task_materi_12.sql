-- Active: 1694033776901@@127.0.0.1@3306@task_12
-- Query dari rancangan database yang sudah dibuat
...
INSERT INTO products (product_type_id, operator_id, code, name, status) VALUES
    (1, 3, 'XA76', 'Aspal Emulsi CRS-1', 2),
    (1, 3, 'XA77', 'Aspal Emulsi CSS-1', 3);

-- d. Insert 3 product dengan product type id = 2, dan operators id = 1.
INSERT INTO products (product_type_id, operator_id, code, name, status) VALUES
    (2, 1, 'BA09', 'Aspal Concreate wearing Course AC WC', 4),
    (2, 1, 'BA10', 'Aspal Concreate Binder Course AC BC', 5),
    (2, 1, 'BA11', 'Aspal Laston', 6);

-- e. Insert 3 product dengan product type id = 3, dan operators id = 4.
INSERT INTO products (product_type_id, operator_id, code, name, status) VALUES
    (3, 4, 'CV06', 'Aspal Dense-Graded Mixes', 7),
    (3, 4, 'CV07', 'Aspal Stone Matrix', 8),
    (3, 4, 'CV08', 'Aspal Open Graded Mixes', 9);

SELECT * FROM products;

-- f. Insert product description pada setiap product.
INSERT INTO product_descriptions (description) VALUES 
    ('Aspal Emulsi CRS-1 - Jenis ini memiliki viskositas yang rendah dan cocok untuk digunakan pada prime coat, pelekat, dan lapis permukaan'),
    ('Aspal Emulsi CSS-1 - Jenis ini memiliki viskositas yang lebih tinggi daripada CRS-1 dan biasanya digunakan untuk pelekat, pekerjaan rembesan, dan pengikat agregat halus pada campuran aspal.'),
    ('Aspal Concreate wearing Course AC WC - adalah jenis campuran aspal yang dirancang khusus untuk digunakan sebagai lapisan permukaan dalam konstruksi jalan dan perkerasan. Produk ini memiliki beberapa karakteristik utama yang membuatnya ideal untuk digunakan sebagai lapisan permukaan dalam proyek infrastruktur jalan'),
    ('Aspal Concreate Binder Course AC BC - adalah jenis campuran aspal yang digunakan sebagai lapisan tengah dalam konstruksi jalan dan perkerasan. Ini adalah komponen kunci dari sistem perkerasan jalan dan memiliki peran utama dalam menambahkan kekuatan struktural dan daya tahan terhadap lalu lintas kendaraan. '),
    ('Aspal Laston - juga dikenal sebagai Hot Mix Asphalt (HMA) atau Asphalt Concrete Surface Course, adalah jenis campuran aspal yang digunakan sebagai lapisan permukaan terakhir atau penutup dalam konstruksi jalan dan perkerasan.'),
    ('Aspal Dense-Graded Mixes - atau sering disebut Dense-Graded Asphalt Concrete, adalah salah satu jenis campuran aspal yang digunakan dalam konstruksi jalan dan perkerasan.'),
    ('Aspal Stone Matrix - adalah jenis campuran aspal khusus yang digunakan dalam konstruksi jalan dan perkerasan. Stone Matrix Asphalt adalah campuran yang didesain untuk memberikan performa lapisan permukaan jalan yang sangat baik dalam berbagai kondisi.'),
    ('Aspal Open Graded Mixes - , adalah jenis campuran aspal yang memiliki karakteristik khusus yang membedakannya dari campuran aspal konvensional. Ini digunakan dalam konstruksi jalan dan perkerasan dengan tujuan utama untuk mempromosikan drainase air yang efektif.');
...
INSERT INTO users (status, name, dob, gender) VALUES
    ('1', 'Maharani Putri', '2002-03-10', 'F'),
    ('1', 'Nurul Nainggolan', '2001-11-12', 'F'),
    ('1', 'Asep Putra Ramadhan', '1998-12-07', 'M'),
    ('1', 'Thamrin Mahaputra', '1985-09-10', 'M'),
    ('1', 'Yogi Perdana', '2001-11-05', 'M');
